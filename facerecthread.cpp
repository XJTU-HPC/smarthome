...
// painting
//----------------------------------------------------------------------------------------
void faceRecThread:: DrawObjects(cv::Mat &frame, vector<FaceObject> &Faces)
{
...
//----- labels ----------------------------
    ...
    this->p_stranger=this->n_stranger;
    if(is_stranger){
        if(this->stranger_cnt<30) this->stranger_cnt++;
        if(this->stranger_cnt>=20) this->n_stranger = true;
    }else{
        if(this->stranger_cnt>0) this->stranger_cnt--;
        if(this->stranger_cnt<=10) this->n_stranger = false;
    }
    if((this->p_stranger==false)&&(this->n_stranger==true)){
        std::cout<<"alarm\n";
        emit alarm();
    }
}
...
faceRecThread::faceRecThread(QObject *parent)
    : QThread{parent}
{
